...
function smtpConfig() {
  ...
  const secure = String(process.env.SMTP_SECURE || "").trim()
    ...
    : portNumber === 465;
  return { host, port: portNumber, user, pass, from, secure };
}
...
async function sendResultEmail(person, attempt) {
  ...
  if (missing.length) {
    attempt.emailStatus = `error: faltan variables SMTP (${missing.join(", ")})`;
    console.error(`[EMAIL] No se envio resultado a ${attempt.email}. Faltan variables en Render: ${missing.join(", ")}`);
    return;
  }
  ...
  try {
    console.log(`[EMAIL] Enviando resultado a ${attempt.email} desde ${config.from} via ${config.host}:${config.port} secure=${config.secure}`);
    ...
    attempt.emailMessageId = info.messageId || "";
    console.log(`[EMAIL] Resultado enviado correctamente a ${attempt.email}. messageId=${info.messageId || "sin-id"}`);
  } catch (error) {
    attempt.emailStatus = `error: ${error.message}`;
    console.error(
      `[EMAIL] Error al enviar resultado a ${attempt.email} desde ${config.from} via ${config.host}:${config.port}. ` +
