...
function enrichContent(content) {
      ...
      evaluations: [
        {
          ...
          questionCount: content.questions.length || examQuestionCount,
          includeSurvey: true,
  ...
  for (const course of content.courses) {
    ...
    for (const evaluation of course.evaluations) {
      ...
      evaluation.questionCount = evaluation.questions.length || Number(evaluation.questionCount || examQuestionCount);
...
function attemptFor(participantId, evaluationId = defaultEvaluationId) {
  const key = attemptKey(participantId, evaluationId);
  const expectedQuestionIds = selectQuestionIds(participantId, evaluationId);
  ...
  if (!state.attempts[key].examQuestionIds) state.attempts[key].examQuestionIds = expectedQuestionIds;
  if (!state.attempts[key].submittedAt && state.attempts[key].examQuestionIds.length !== expectedQuestionIds.length) {
    state.attempts[key].examQuestionIds = expectedQuestionIds;
  }
...
function selectQuestionIds(seedText, evaluationId = defaultEvaluationId) {
  ...
  return shuffled.slice(0, Math.min(evaluation.questions?.length || evaluation.questionCount || examQuestionCount, shuffled.length));
}
...
function gradeAttempt(attempt) {
  ...
  return {
    correct,
    total: ids.length,
    percent: ids.length ? Math.round((correct / ids.length) * 100) : 0,
    grade10: ids.length ? Number(((correct / ids.length) * 10).toFixed(1)) : 0,
  };
}
          ...
          <table role="presentation" width="760" cellspacing="0" cellpadding="0" style="max-width:760px;width:100%;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 22px 60px rgba(31,52,92,.16);">
            ...
            <tr>
              <td style="padding:26px 30px;">
                <div style="margin:0 0 16px;padding:16px 18px;border:1px solid #dce8f7;border-radius:14px;background:#f8fbff;">
                  <div style="font-size:13px;text-transform:uppercase;letter-spacing:.06em;color:#61708c;font-weight:800;">Participante evaluado</div>
                  <div style="font-size:24px;line-height:1.15;color:#14213d;font-weight:900;">${escapeHtml(person.name)}</div>
                  <div style="font-size:14px;color:#4d5e78;font-weight:700;">${escapeHtml(person.folio)} · ${escapeHtml(person.site)} · ${escapeHtml(person.career)}</div>
                </div>
