...
let selectedEvaluationId = "ultrasonido-final";
...
async function refreshContent() {
  ...
  const evaluations = adminEvaluations();
  if (!evaluations.find((item) => item.evaluation.id === selectedEvaluationId)) selectedEvaluationId = evaluations[0]?.evaluation.id || "ultrasonido-final";
...
function adminCourses() {
  return examData.courses?.length
    ? examData.courses
    : [
        {
          id: "ultrasonido",
          title: examData.course || "Curso Introductorio de Ultrasonido",
          evaluations: [
            {
...
function adminEvaluations() {
  return adminCourses().flatMap((course) => (course.evaluations || []).map((evaluation) => ({ course, evaluation })));
}

function selectedEvaluation() {
  return adminEvaluations().find((item) => item.evaluation.id === selectedEvaluationId)?.evaluation || adminEvaluations()[0]?.evaluation || { questions: [], participants: [] };
}
      ...
      return `
        <tr>
          ...
          <td>
            ...
            <span>${row.answered}/${row.assigned || latestState.examQuestions} reactivos · ${row.surveyAnswered}/${latestState.surveyQuestions} encuesta</span>
          </td>
          <td>${score}</td>
...
function renderQuestionEditor() {
  const evaluations = adminEvaluations();
  $("questionEvaluationSelect").innerHTML = evaluations
    .map(({ course, evaluation }) => `<option value="${evaluation.id}">${course.title} · ${evaluation.title}</option>`)
    .join("");
  $("questionEvaluationSelect").value = selectedEvaluationId;
  const questions = selectedEvaluation().questions || [];
  $("questionCountAdmin").textContent = `${questions.length} reactivos`;
...
function renderParticipantEditor() {
  const evaluations = adminEvaluations();
  $("participantEvaluationSelect").innerHTML = evaluations
    .map(({ course, evaluation }) => `<option value="${evaluation.id}">${course.title} · ${evaluation.title}</option>`)
    .join("");
  $("participantEvaluationSelect").value = selectedEvaluationId;
  const participants = selectedEvaluation().participants || [];
  $("participantCountAdmin").textContent = `${participants.length} participantes`;
  $("participantAdminList").innerHTML = participants
    .map(
      (person) => `
      <article class="editor-item">
        <div>
...
function exportCsv() {
  ...
  const rows = [
    [
      ...
      "aciertos",
      "porcentaje",
      ...selectedEvaluation().questions.map((question) => `R${question.id}`),
  ...
  for (const row of latestState.rows) {
    rows.push([
      ...
      row.score?.correct ?? "",
      row.score?.percent ?? "",
      ...selectedEvaluation().questions.map((question) => row.answers?.[question.id] || ""),
...
$("newCourseForm").addEventListener("submit", async (event) => {
  ...
  examData = await api("/api/admin/content", {
    ...
    course: $("courseNameInput").value,
    examTitle: $("courseExamTitle").value,
    newCourseTitle: $("newCourseTitle").value,
    newEvaluationTitle: $("newEvaluationTitle").value,
  });
  $("newCourseForm").reset();
  await refreshContent();
});
...
$("questionForm").addEventListener("submit", async (event) => {
  ...
  examData = await api("/api/admin/question", {
    evaluationId: $("questionEvaluationSelect").value,
    id: $("questionId").value,
...
$("participantForm").addEventListener("submit", async (event) => {
  ...
  examData = await api("/api/admin/participant", {
    evaluationId: $("participantEvaluationSelect").value,
...
document.addEventListener("click", async (event) => {
  ...
  if (questionId) {
    const question = selectedEvaluation().questions.find((item) => Number(item.id) === Number(questionId));
  ...
  if (deleteQuestion) {
    examData = await api("/api/admin/delete-question", { id: deleteQuestion, evaluationId: selectedEvaluationId });
    await refreshContent();
  }

  if (participantId) {
    const person = selectedEvaluation().participants.find((item) => item.id === participantId);
  ...
  if (deleteParticipant) {
    examData = await api("/api/admin/delete-participant", { id: deleteParticipant, evaluationId: selectedEvaluationId });
    await refreshContent();
  }
});

$("questionEvaluationSelect").addEventListener("change", () => {
  selectedEvaluationId = $("questionEvaluationSelect").value;
  $("questionForm").reset();
  renderQuestionEditor();
  renderParticipantEditor();
});

$("participantEvaluationSelect").addEventListener("change", () => {
  selectedEvaluationId = $("participantEvaluationSelect").value;
  $("participantForm").reset();
  renderQuestionEditor();
  renderParticipantEditor();
});
